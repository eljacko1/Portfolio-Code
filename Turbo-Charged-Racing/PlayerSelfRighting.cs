...
{
    public class PlayerSelfRighting : MonoBehaviour
    {
        ...
        public void OnResetCarOrientation(InputAction.CallbackContext context)
        {
            resetCar = context.ReadValueAsButton();

            if (resetCar)
            {
                if (transform.up.y < 20f)
                {
                    RightCar();
                }     
            }
